...
export default function DcDailyChecklistForm() {
  const { id: editId } = useParams();
  const isEdit = Boolean(editId);
  const navigate = useNavigate();
  const [form, setForm] = useState(() => emptyDcDailyForm());
  ...
  useEffect(() => {
    if (!editId) return;
    hsmAPI.getDcDailyChecklistById(editId)
      .then((res) => {
        const d = res?.data || res;
        if (!d?.can_modify && isEdit) {
          toast.error('Edit window expired (10 hours)');
          navigate(`/hsm/dc-daily-checklist/${editId}`);
          return;
        }
        const base = emptyDcDailyForm();
      ...
      .catch(() => {
        ...
        navigate('/hsm/dc-daily-checklist/history');
      })
  ...
  const validate = () => {
    ...
    for (const { key, sectionLabel } of DC_CHECK_ITEMS) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await hsmAPI.updateDcDailyChecklist(editId, fd);
        toast.success('Checklist updated');
        navigate(`/hsm/dc-daily-checklist/${editId}`);
      } else {
        const res = await hsmAPI.createDcDailyChecklist(fd);
        const id = res?.data?.id;
        toast.success('Checklist saved');
        navigate(id ? `/hsm/dc-daily-checklist/${id}` : '/hsm/dc-daily-checklist/history');
      }
    } catch (e) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <h1 className="text-2xl font-bold text-gray-900">
            {isEdit ? 'Edit DC Daily Check List' : 'DC Daily Check List'}
          </h1>
          <Link
        ...
        <div className="mb-5 space-y-5">
          ...
          {DC_CHECK_SECTIONS.map((section, sIdx) => {

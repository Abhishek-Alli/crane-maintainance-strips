...
export default function InductionDailyChecklistView() {
  const { id } = useParams();
  ...
  useEffect(() => {
    hsmAPI.getInductionDailyChecklistById(id)
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      const res = await hsmAPI.downloadInductionDailyChecklistPDF(id);
      const blob = new Blob([res.data], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `hsm_induction_daily_checklist_${id}.pdf`;
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await hsmAPI.deleteInductionDailyChecklist(id);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <div className="max-w-4xl mx-auto">
        <button
          type="button"
          onClick={() => navigate('/hsm/induction-daily-checklist/history')}
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Induction Daily Check List</h1>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {canModify && (
              <>
                <button
                  type="button"
                  onClick={() => navigate(`/hsm/induction-daily-checklist/${id}/edit`)}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {INDUCTION_CHECK_ITEMS.map(({ key, label }, i) => {
                  const row = items[key] || {};
                  const st = row.status;
                  return (

...
const SECTION_META = {
  'SECTION-1': { label: 'Section 1 — Twin Channel',        color: 'bg-emerald-600', remarkKey: 'sec1_remark', resultKey: 'sec1_result', checkedKey: 'sec1_checked_by' },
  'SECTION-2': { label: 'Section 2 — Rake',                color: 'bg-blue-600',    remarkKey: 'sec2_remark', resultKey: 'sec2_result', checkedKey: 'sec2_checked_by' },
  'SECTION-3': { label: 'Section 3 — Aligner Roller',      color: 'bg-purple-600',  remarkKey: 'sec3_remark', resultKey: 'sec3_result', checkedKey: 'sec3_checked_by' },
  'SECTION-4': { label: 'Section 4 — Layer Shifting Tray', color: 'bg-orange-600',  remarkKey: 'sec4_remark', resultKey: 'sec4_result', checkedKey: 'sec4_checked_by' },
  'SECTION-5': { label: 'Section 5 — Roller Conveyor',     color: 'bg-teal-600',    remarkKey: 'sec5_remark', resultKey: 'sec5_result', checkedKey: 'sec5_checked_by' },
  'SECTION-6': { label: 'Section 6 — Cold Shear',          color: 'bg-rose-600',    remarkKey: 'sec6_remark', resultKey: 'sec6_result', checkedKey: 'sec6_checked_by' },
};

const CoolingBedView = () => {
  const { id } = useParams();
  ...
  const fetchLog = async () => {
    try {
      const res = await hbmAPI.getCoolingBedLogById(id);
      setLog(res.data);
    } catch {
      toast.error('Failed to load Cooling Bed log');
      navigate('/hbm/cooling-bed/history');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            {canEdit && (
              <button
                onClick={() => navigate(`/hbm/cooling-bed/new`, { state: { editData: log, editId: log.id } })}
            ...
            <ResendTelegramButton type="cooling-bed" id={log.id} />
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
            ...
            <p className="text-xs text-gray-500">Twin Ch · Rake · Aligner · Tray · Conv · Shear</p>
          </div>
        </div>

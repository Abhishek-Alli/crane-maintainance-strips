...
const WaterParamHistory = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await hbmAPI.deleteLog('water-param', id);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Water Parameter History</h1>
              <p className="text-sm text-gray-500">{logs.length} records</p>
            </div>
          </div>
          <Link to="/hbm/water-param/new"
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-16 text-center">
            <p className="text-gray-400 text-lg">No Water Parameter logs found</p>
            <Link to="/hbm/water-param/new" className="mt-4 inline-block text-blue-600 font-semibold hover:underline">
              Submit first report
            </Link>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left px-5 py-3 font-semibold text-gray-600">Sources</th>
                  <th className="text-left px-5 py-3 font-semibold text-gray-600">Remark</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {logs.map(log => (
                  <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-gray-600">{log.total_entries} sources</td>
                    <td className="px-5 py-3 text-gray-500 text-xs max-w-[200px] truncate">{log.remark || '—'}</td>
                    <td className="px-5 py-3 flex items-center gap-3">
                      <Link to={`/hbm/water-param/${log.id}`}
...
export default WaterParamHistory;

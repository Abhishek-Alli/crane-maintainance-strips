  ...
  const handleSubmit = async (e) => {
      ...
      for (const [secKey, secData] of Object.entries(SHEET_CONFIG)) {
        for (const [block, blockItems] of Object.entries(secData.blocks)) {
          for (const item of blockItems) {
            ...
            items.push({ section_name: secKey, block_name: block, item_name: item,
              status: val.status, remark: val.remark || '', action_taken: val.action_taken || '' });
          }
        }
      }
    } catch (err) {
    ...
    try {
      await hbmAPI.createCoolingBedLog({
        ...
        sec6_remark: footers['SECTION-6'].remark || null,
        sec6_result: footers['SECTION-6'].result || null,
        sec6_checked_by: footers['SECTION-6'].checked_by || null,
        items,
      });

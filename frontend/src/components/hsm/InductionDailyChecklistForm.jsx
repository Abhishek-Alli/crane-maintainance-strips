...
import {
  INDUCTION_CHECK_ITEMS,
  emptyInductionDailyForm,
  emptySectionValues,
} from './inductionDailyConfig';
...
export default function InductionDailyChecklistForm() {
  const { id: editId } = useParams();
  const isEdit = Boolean(editId);
  const navigate = useNavigate();
  const [form, setForm] = useState(() => emptyInductionDailyForm());
  ...
  useEffect(() => {
    if (!editId) return;
    hsmAPI.getInductionDailyChecklistById(editId)
      .then((res) => {
        const d = res?.data || res;
        if (!d?.can_modify && isEdit) {
          toast.error('Edit window expired (10 hours)');
          navigate(`/hsm/induction-daily-checklist/${editId}`);
          return;
        }
        const base = emptyInductionDailyForm();
      ...
      .catch(() => {
        ...
        navigate('/hsm/induction-daily-checklist/history');
      })
  ...
  const validate = () => {
    ...
    for (const { key, label } of INDUCTION_CHECK_ITEMS) {
      const row = form.checklist_items[key] || {};
      if (row.status === 'NOT_OK' && !String(row.action_taken || '').trim()) {
        return `Action taken is required for "${label}" (marked NOT OK)`;
      }
    }
    return null;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await hsmAPI.updateInductionDailyChecklist(editId, fd);
        toast.success('Checklist updated');
        navigate(`/hsm/induction-daily-checklist/${editId}`);
      } else {
        const res = await hsmAPI.createInductionDailyChecklist(fd);
        const id = res?.data?.id;
        toast.success('Checklist saved');
        navigate(id ? `/hsm/induction-daily-checklist/${id}` : '/hsm/induction-daily-checklist/history');
      }
    } catch (e) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <h1 className="text-2xl font-bold text-gray-900">
            {isEdit ? 'Edit Induction Daily Check List' : 'Induction Daily Check List'}
          </h1>
          <Link
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-5 space-y-5">
          ...
          {INDUCTION_CHECK_ITEMS.map(({ key, label }, idx) => {
                  ...
                  <div className="mt-3">
                    <Field label="Action taken" required>
                      <input
                        ...
                        onChange={(e) => setItem(key, { action_taken: e.target.value })}

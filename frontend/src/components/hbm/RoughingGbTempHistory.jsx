...
const RoughingGbTempHistory = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await hbmAPI.getRoughingGbTempLogs(params);
      setLogs(res.data);
    } catch {
      toast.error('Failed to load Roughing GB Temp logs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await hbmAPI.deleteLog('roughing-gb-temp', id);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Roughing GB Temp History</h1>
              <p className="text-sm text-gray-500">{logs.length} records</p>
            </div>
          </div>
          <Link to="/hbm/roughing-gb-temp/new"
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-16 text-center">
            <p className="text-gray-400 text-lg">No Roughing GB Temp logs found</p>
            <Link to="/hbm/roughing-gb-temp/new" className="mt-4 inline-block text-blue-600 font-semibold hover:underline">
              Submit first report
            </Link>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left px-5 py-3 font-semibold text-gray-600">Shift Eng</th>
                  <th className="text-left px-5 py-3 font-semibold text-gray-600">Temp Taken By</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {logs.map(log => (
                  <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-gray-600">{log.shift_eng || '—'}</td>
                    <td className="px-5 py-3 text-gray-600">{log.temp_taken_by || '—'}</td>
                    <td className="px-5 py-3 text-gray-600">{log.filled_by_name}</td>
                    <td className="px-5 py-3 flex items-center gap-3">
                      <Link to={`/hbm/roughing-gb-temp/${log.id}`}
...
export default RoughingGbTempHistory;

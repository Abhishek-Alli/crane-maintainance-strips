...
const SECTION_META = {
  'SECTION-1': { label: 'Section 1 — Rolling Stands C1–C14',        color: 'bg-blue-700',   resultKey: 'sec1_result' },
  'SECTION-2': { label: 'Section 2 — Loopers & Snapshears',         color: 'bg-teal-600',   resultKey: 'sec2_result' },
  'SECTION-3': { label: 'Section 3 — Pinch Rolls & Tail Breakers',  color: 'bg-orange-600', resultKey: 'sec3_result' },
  'SECTION-4': { label: 'Section 4 — Flying / Continue Shear',      color: 'bg-rose-600',   resultKey: 'sec4_result' },
};

const BeforeRollingView = () => {
  const { id } = useParams();
  ...
  const fetchLog = async () => {
    try {
      const res = await hbmAPI.getBeforeRollingLogById(id);
      setLog(res.data);
    } catch {
      toast.error('Failed to load Before Rolling log');
  ...
  const handleDownloadPDF = async () => {
    setDownloading(true);
    try {
      const res = await hbmAPI.downloadPDF('before-rolling', id);
      const url = URL.createObjectURL(res.data);
      const a = document.createElement('a');
      a.href = url;
      a.download = `hbm_before_rolling_${id}.pdf`;
      a.click();
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            <button onClick={() => navigate('/hbm/before-rolling/history')}
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Before Rolling Log #{log.id}</h1>
              <p className="text-sm text-gray-500">HBM Before Rolling Checksheet</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {canEdit && (
              <button
                onClick={() => navigate(`/hbm/before-rolling/new`, { state: { editData: log, editId: log.id } })}
            ...
            <ResendTelegramButton type="before-rolling" id={log.id} />
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-5 grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div>
            <p className="text-xs text-gray-400 uppercase font-semibold">Mill Shift Incharge</p>
            <p className="font-bold text-gray-900 mt-1">{log.mill_shift_incharge || '—'}</p>
          </div>
          <div>
            <p className="text-xs text-gray-400 uppercase font-semibold">Mechanical Engineer</p>
            <p className="font-bold text-gray-900 mt-1">{log.mechanical_engineer || '—'}</p>
          </div>
        </div>

        {/* Filled By */}
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-5">
          <p className="text-xs text-gray-400 uppercase font-semibold">Filled By (System)</p>
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
            ...
            <p className="text-xs text-gray-500">Stands · Loopers · Pinch · Shear</p>
          </div>
        </div>
                ...
                <div key={item.id} className="bg-white rounded-lg border border-red-200 p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      ...
                      {item.item_value && (
                        <p className="text-xs text-gray-600 mt-0.5">Value: <span className="font-semibold">{item.item_value}</span></p>
                      )}
                    </div>
                      ...
                      <div key={item.id}
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            {item.item_value && (
                              <p className="text-xs text-gray-500 mt-0.5">Value: <span className="font-semibold text-gray-700">{item.item_value}</span></p>
                            )}
                          </div>
...
export default BeforeRollingView;

...
const RollingStandHistory = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await hbmAPI.getRollingStandLogs(params);
      setLogs(res.data);
    } catch {
      toast.error('Failed to load Rolling Stand logs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await hbmAPI.deleteLog('rolling-stand', id);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Rolling Stand History</h1>
              <p className="text-sm text-gray-500">{logs.length} records</p>
            </div>
          </div>
          <Link to="/hbm/rolling-stand/new"
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-16 text-center">
            <p className="text-gray-400 text-lg">No Rolling Stand logs found</p>
            <Link to="/hbm/rolling-stand/new" className="mt-4 inline-block text-emerald-600 font-semibold hover:underline">
                ...
                {logs.map(log => (
                  <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3 flex items-center gap-3">
                      <Link to={`/hbm/rolling-stand/${log.id}`}
...
export default RollingStandHistory;

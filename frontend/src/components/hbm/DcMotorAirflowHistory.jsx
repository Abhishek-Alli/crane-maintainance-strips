...
const DcMotorAirflowHistory = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await hbmAPI.getDcMotorAirflowLogs(params);
      setLogs(res.data);
    } catch {
      toast.error('Failed to load DC Motor Airflow logs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await hbmAPI.deleteLog('dc-motor-airflow', id);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">DC Motor Airflow History</h1>
              <p className="text-sm text-gray-500">{logs.length} records</p>
            </div>
          </div>
          <Link to="/hbm/dc-motor-airflow/new"
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-16 text-center">
            <p className="text-gray-400 text-lg">No DC Motor Airflow logs found</p>
            <Link to="/hbm/dc-motor-airflow/new" className="mt-4 inline-block text-blue-600 font-semibold hover:underline">
              Submit first report
            </Link>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="text-left px-5 py-3 font-semibold text-gray-600">Reading By</th>
                ...
                {logs.map(log => (
                  <tr key={log.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3 flex items-center gap-3">
                      <Link to={`/hbm/dc-motor-airflow/${log.id}`}
...
export default DcMotorAirflowHistory;

...
export default function RmDailyChecklistForm() {
  const { id: editId } = useParams();
  const isEdit = Boolean(editId);
  const navigate = useNavigate();
  const [form, setForm] = useState(() => emptyRmDailyForm());
  ...
  useEffect(() => {
    if (!editId) return;
    hsmAPI.getRmDailyChecklistById(editId)
      .then((res) => {
        const d = res?.data || res;
        if (!d?.can_modify && isEdit) {
          toast.error('Edit window expired (10 hours)');
          navigate(`/hsm/rm-daily-checklist/${editId}`);
          return;
        }
        const base = emptyRmDailyForm();
      ...
      .catch(() => {
        ...
        navigate('/hsm/rm-daily-checklist/history');
      })
  ...
  const validate = () => {
    ...
    for (const { key, sectionLabel } of RM_CHECK_ITEMS) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await hsmAPI.updateRmDailyChecklist(editId, fd);
        toast.success('Checklist updated');
        navigate(`/hsm/rm-daily-checklist/${editId}`);
      } else {
        const res = await hsmAPI.createRmDailyChecklist(fd);
        const id = res?.data?.id;
        toast.success('Checklist saved');
        navigate(id ? `/hsm/rm-daily-checklist/${id}` : '/hsm/rm-daily-checklist/history');
      }
    } catch (e) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <h1 className="text-2xl font-bold text-gray-900">
            {isEdit ? 'Edit RM Daily Check List' : 'RM Daily Check List'}
          </h1>
          <Link
        ...
        <div className="mb-5 space-y-5">
          ...
          {RM_CHECK_SECTIONS.map((section, sIdx) => {

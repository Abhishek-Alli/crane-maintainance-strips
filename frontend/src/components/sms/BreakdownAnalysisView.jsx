...
const Section = ({ title, children, flush }) => (
  <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5 mb-5">
    <h2 className="text-sm font-bold text-amber-800 uppercase tracking-wide mb-4 border-b border-amber-100 pb-2">
      {title}
    </h2>
    ...
    <table className="w-full text-sm">
      <tbody className="divide-y divide-gray-100">
        {rows.map(([label, value]) => (
          <tr key={label} className="odd:bg-amber-50/40">
    ...
    <table className="w-full text-sm">
      ...
      <tbody className="divide-y divide-gray-100">
        {rows.map(([n, problem, dueTo]) => (
          <tr key={n} className="odd:bg-amber-50/30">
            <td className="px-3 py-2.5 font-bold text-amber-800 align-top whitespace-nowrap">WHY - {n}</td>
...
export default function BreakdownAnalysisView() {
  ...
  useEffect(() => {
    smsAPI.getBreakdownAnalysisById(id)
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      const res = await smsAPI.downloadBreakdownAnalysisPDF(id);
      const blob = new Blob([res.data], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `sms_breakdown_analysis_${id}.pdf`;
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await smsAPI.deleteBreakdownAnalysis(id);
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
          <div className="flex items-center space-x-3">
            <button
              type="button"
              onClick={() => navigate('/sms/breakdown-analysis/history')}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {canModify && (
              <button
                type="button"
                onClick={() => navigate(`/sms/breakdown-analysis/${id}/edit`)}
                className="px-3 py-1.5 bg-white border border-amber-300 text-amber-800 rounded-lg text-xs font-semibold hover:bg-amber-50"
              >
                Edit
              </button>
            )}

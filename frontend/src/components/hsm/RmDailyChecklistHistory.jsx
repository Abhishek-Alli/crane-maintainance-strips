...
import { RM_CHECK_ITEMS } from './rmDailyConfig';
...
function countNotOk(items) {
  ...
  return RM_CHECK_ITEMS.filter(({ key }) => items[key]?.status === 'NOT_OK').length;
}

export default function RmDailyChecklistHistory() {
  ...
  const fetchLogs = async (from = dateFrom, to = dateTo) => {
    ...
    try {
      ...
      const res = await hsmAPI.getRmDailyChecklistLogs(params);
      setLogs(res?.data || []);
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm('Delete ALL RM Daily Check List history?\n\nThis permanently removes every record from the database and cannot be undone.')) {
      return;
    }
    if (!window.confirm('Final confirmation: delete the entire RM Daily Check List history?')) {
      return;
    }
    setDeletingAll(true);
    try {
      const res = await hsmAPI.clearAllRmDailyChecklists();
      toast.success(res?.message || 'All RM Daily checklists deleted');
      setLogs([]);
      setTotal(0);
    } catch {
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900">RM Daily Check List History</h1>
          </div>
                      ...
                      return (
                        <tr key={log.id} className="hover:bg-indigo-50/40 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right">
                            <Link
                              to={`/hsm/rm-daily-checklist/${log.id}`}

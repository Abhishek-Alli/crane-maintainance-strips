...
} = require('../utils/hsmDcDailyConfig');

const saveDcDailyImages = makeImageSaver('hsm_dc_daily_images', 'hsm-dc-daily');
...
class HsmDcDailyController {
    ...
    try {
      ...
      const result = await query(
        `SELECT l.*, u.username AS filled_by_name
         FROM hsm_dc_daily_checklists l
      ...
      console.error('HSM DC Daily getLogs error:', error);
      res.status(500).json({ success: false, message: 'Failed to fetch DC Daily checklists' });
    }
  }
  ...
  static async getById(req, res) {
    try {
      ...
      const [result, imagesRes] = await Promise.all([
        ...
        query(`SELECT * FROM hsm_dc_daily_images WHERE log_id = $1 ORDER BY sort_order, id`, [id]),
      ]);
      if (!result.rows.length) {
        return res.status(404).json({ success: false, message: 'DC Daily checklist not found' });
      }
      ...
      console.error('HSM DC Daily getById error:', error);
      res.status(500).json({ success: false, message: 'Failed to fetch DC Daily checklist' });
    }
  }
  ...
  static async create(req, res) {
    ...
    try {
      ...
      const err = HsmDcDailyController._validate(b);
      if (err) {
      ...
      await transaction(async (client) => {
        const result = await client.query(
          `INSERT INTO hsm_dc_daily_checklists (
        ...
        log = result.rows[0];
        await saveDcDailyImages(client, log.id, uploaded, null);
      });

      res.status(201).json({
        success: true,
        message: 'DC Daily checklist saved',
        data: log,
      });
    } catch (error) {
      cleanupUploadedFiles(uploaded);
      console.error('HSM DC Daily create error:', error);
      res.status(500).json({ success: false, message: 'Failed to save DC Daily checklist' });
    }
  }
  ...
  static async update(req, res) {
    ...
    try {
      const { id } = req.params;
      const existing = await query(`SELECT * FROM hsm_dc_daily_checklists WHERE id = $1`, [id]);
      ...
      await transaction(async (client) => {
        const result = await client.query(
          `UPDATE hsm_dc_daily_checklists SET
        ...
        log = result.rows[0];
        await saveDcDailyImages(client, log.id, uploaded, keepImageIds);
      });

      res.json({
        success: true,
        message: 'DC Daily checklist updated',
        data: log,
      });
    } catch (error) {
      cleanupUploadedFiles(uploaded);
      console.error('HSM DC Daily update error:', error);
      res.status(500).json({ success: false, message: 'Failed to update DC Daily checklist' });
    }
  }
  ...
  static async remove(req, res) {
    try {
      ...
      const images = await query(`SELECT file_path FROM hsm_dc_daily_images WHERE log_id = $1`, [id]);
      await query(`DELETE FROM hsm_dc_daily_checklists WHERE id = $1`, [id]);
      images.rows.forEach((img) => unlinkUpload(img.file_path));
      res.json({ success: true, message: 'DC Daily checklist deleted' });
    } catch (error) {
      console.error('HSM DC Daily delete error:', error);
      res.status(500).json({ success: false, message: 'Failed to delete DC Daily checklist' });
    }
  }

  static async clearAll(req, res) {
    try {
      const images = await query(`SELECT file_path FROM hsm_dc_daily_images`);
      const result = await query(`DELETE FROM hsm_dc_daily_checklists RETURNING id`);
      images.rows.forEach((img) => unlinkUpload(img.file_path));
      res.json({
        success: true,
        message: `Deleted ${result.rowCount} DC Daily checklist(s)`,
        deleted: result.rowCount,
      });
    } catch (error) {
      console.error('HSM DC Daily clearAll error:', error);
      res.status(500).json({ success: false, message: 'Failed to delete all DC Daily history' });
    }
  }
  ...
  static async downloadPDF(req, res) {
    ...
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename=hsm_dc_daily_checklist_${id}_${log.report_date || 'report'}.pdf`
      );
      ...
      doc.font('Helvetica-Bold').fontSize(14).fillColor('#1e1b4b')
        .text('DC Daily Check List', margin + 60, y + 8, { width: pageW - 60 });
      ...
      console.error('HSM DC Daily PDF error:', error);
      if (!res.headersSent) {
...
module.exports = HsmDcDailyController;

...
export default function DcDailyChecklistView() {
  const { id } = useParams();
  ...
  useEffect(() => {
    hsmAPI.getDcDailyChecklistById(id)
  ...
  const handleDownload = async () => {
    setDownloading(true);
    try {
      const res = await hsmAPI.downloadDcDailyChecklistPDF(id);
      const blob = new Blob([res.data], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `hsm_dc_daily_checklist_${id}.pdf`;
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await hsmAPI.deleteDcDailyChecklist(id);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <div className="max-w-4xl mx-auto">
        <button
          type="button"
          onClick={() => navigate('/hsm/dc-daily-checklist/history')}
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">DC Daily Check List</h1>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {canModify && (
              <>
                <button
                  type="button"
                  onClick={() => navigate(`/hsm/dc-daily-checklist/${id}/edit`)}

...
import { INDUCTION_CHECK_ITEMS } from './inductionDailyConfig';
...
function countNotOk(items) {
  ...
  return INDUCTION_CHECK_ITEMS.filter(({ key }) => items[key]?.status === 'NOT_OK').length;
}

export default function InductionDailyChecklistHistory() {
  ...
  const fetchLogs = async (from = dateFrom, to = dateTo) => {
    ...
    try {
      ...
      const res = await hsmAPI.getInductionDailyChecklistLogs(params);
      setLogs(res?.data || []);
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm('Delete ALL Induction Daily Check List history?\n\nThis permanently removes every record from the database and cannot be undone.')) {
      return;
    }
    if (!window.confirm('Final confirmation: delete the entire Induction Daily Check List history?')) {
      return;
    }
    setDeletingAll(true);
    try {
      const res = await hsmAPI.clearAllInductionDailyChecklists();
      toast.success(res?.message || 'All Induction Daily checklists deleted');
      setLogs([]);
      setTotal(0);
    } catch {
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900">Induction Daily Check List History</h1>
          </div>
                    ...
                    {logs.map((log, i) => {
                      ...
                      return (
                        <tr key={log.id} className="hover:bg-indigo-50/40 transition-colors">
                          ...
                          <td className="px-4 py-3 text-gray-700">{log.operator_name || '—'}</td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3 text-right">
                            <Link
                              to={`/hsm/induction-daily-checklist/${log.id}`}

...
const inputCls =
  'w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent';
...
const WhyBlock = ({ n, form, set }) => (
  <div className="rounded-lg border border-amber-100 bg-amber-50/40 p-4 space-y-3">
    <p className="text-sm font-bold text-amber-800">WHY - {n}</p>
...
export default function BreakdownAnalysisForm() {
  ...
  useEffect(() => {
    if (!editId) return;
    setLoadingEdit(true);
    smsAPI.getBreakdownAnalysisById(editId)
      .then((res) => {
        ...
        if (!isWithinEditWindow(log.created_at) && !log.can_modify) {
          ...
          navigate(`/sms/breakdown-analysis/${editId}`);
          return;
        }
      ...
      .catch((err) => {
        ...
        navigate('/sms/breakdown-analysis/history');
      })
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      let newId = editId;
      if (isEdit) {
        await smsAPI.updateBreakdownAnalysis(editId, payload);
        toast.success('Breakdown Analysis Report updated');
      } else {
        const res = await smsAPI.createBreakdownAnalysis(payload);
        newId = res?.data?.id || res?.id;
        toast.success('Breakdown Analysis Report submitted');
      }
      navigate(newId ? `/sms/breakdown-analysis/${newId}` : '/sms/breakdown-analysis/history');
    } catch (err) {
  ...
  return (
    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center space-x-3 mb-6">
          <button
            type="button"
            onClick={() => navigate(isEdit ? `/sms/breakdown-analysis/${editId}` : '/sms/dashboard')}
        ...
        <form onSubmit={handleSubmit}>
          <Section title="Basic Details">
            ...
            <div className="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3">
              <p className="text-xs font-semibold text-amber-700 uppercase">Total Downtime</p>
              <p className="text-lg font-bold text-amber-900 mt-0.5">{formatDowntime(downtimeMins)}</p>
              <p className="text-xs text-amber-600 mt-1">Auto-calculated from breakdown & restoration times</p>
            </div>
          </Section>
          ...
          <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4 shadow-lg rounded-t-xl">
            <button
              type="submit"
              disabled={submitting}
              className="w-full py-3 bg-amber-600 text-white rounded-lg font-bold text-base hover:bg-amber-700 transition-colors disabled:opacity-60"
            >
    ...
    >
      <PreviewSection title="Basic Details" color="bg-amber-700">
        <PreviewGrid
          rows={[
      ...
      <PreviewSection title="Breakdown Description" color="bg-amber-700">
        <PreviewGrid
          rows={[
      ...
      <PreviewSection title="Root Cause Analysis" color="bg-amber-700">
        <PreviewGrid
          rows={[
      ...
      <PreviewSection title="5-Why Analysis" color="bg-amber-700">
        <div className="space-y-2 text-sm">
          {[1, 2, 3, 4, 5].map((n) => (
            <div key={n} className="border border-gray-100 rounded-lg p-2">
              <p className="font-semibold text-amber-800 text-xs mb-1">WHY - {n}</p>
      ...
      <PreviewSection title="Corrective / Preventive / Store" color="bg-amber-700">
        <PreviewGrid
          rows={[
      ...
      <PreviewSection title="Sign-off" color="bg-amber-700">
        <PreviewGrid
          rows={[

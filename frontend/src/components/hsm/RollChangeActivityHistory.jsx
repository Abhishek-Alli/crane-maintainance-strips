...
export default function RollChangeActivityHistory() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await hsmAPI.getRollChangeActivityLogs(params);
      setLogs(res?.data || []);
    } catch {
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm('Delete ALL Roll Change Activity history?\n\nThis permanently removes every roll change report (and images) from the database and cannot be undone.')) {
      return;
    }
    if (!window.confirm('Final confirmation: delete the entire Roll Change Activity sheet history?')) {
      return;
    }
    setDeletingAll(true);
    try {
      const res = await hsmAPI.clearAllRollChangeActivity();
      toast.success(res?.message || 'All roll change activity reports deleted');
      setLogs([]);
    } catch {
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900">Roll Change Activity History</h1>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-3 mb-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button type="button" onClick={() => { setDateFrom(''); setDateTo(''); setTimeout(fetchLogs, 0); }}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Shift</th>
                    <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Area</th>
                    <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Equipment</th>
                    <th className="text-left px-5 py-3 text-xs font-semibold text-gray-500 uppercase">Manpower</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {logs.map((log, i) => (
                    <tr key={log.id} className="hover:bg-indigo-50/40 transition-colors">
                      ...
                      <td className="px-5 py-3 font-semibold text-gray-900 whitespace-nowrap">{formatDate(log.report_date)}</td>
                      <td className="px-5 py-3">
                        <span className="bg-indigo-100 text-indigo-800 text-xs font-semibold px-2 py-0.5 rounded">
                          {log.shift || '—'}
                        </span>
                      </td>
                      <td className="px-5 py-3 text-gray-800">{log.area || '—'}</td>
                      <td className="px-5 py-3 text-gray-700">{log.equipment_count != null ? log.equipment_count : '—'}</td>
                      <td className="px-5 py-3 text-gray-700">{log.manpower_count != null ? log.manpower_count : '—'}</td>
